...
```{r }
unique(DF$Defendant.Race)
rrw = sum(DF$Defendant.Race == "WHITE")
rrb = sum(DF$Defendant.Race == "BLACK")
rra = sum(DF$Defendant.Race == "ASIAN")
rrw
rrb
rra
```
...
```{r }
ss = subset(DF,DF$Arrest.Category == "Sex Abuse")
HH = ggplot(ss, aes(group = factor(Arrest.Year), y = Arrest.Hour,x= Arrest.Year, fill = factor(Arrest.Year))) + 
  geom_boxplot() + 
  geom_boxplot(outlier.shape=8, outlier.size=5) +
  labs(title="Arrest.Hour VS Year", x="Year", y = "Arrest.Hour")
HH
```
...
```{r }
tt = subset(DF,DF$Arrest.Category == "Theft")
HHH = ggplot(tt, aes(group = factor(Arrest.Year), y = Arrest.Hour,x= Arrest.Year, fill = factor(Arrest.Year))) + 
  geom_boxplot() + 
  geom_boxplot(outlier.shape=8, outlier.size=5) +
  labs(title="Arrest.Hour VS Year", x="Year", y = "Arrest.Hour")
HHH
